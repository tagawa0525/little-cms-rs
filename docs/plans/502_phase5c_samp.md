...
| ファイル | 操作 |
| --- | --- |
| `src/transform/samp.rs` | 新規作成: 黒点検出 + `formatter_for_colorspace()` |
...
- `Profile::is_intent_supported()` — 現時点では不要（`is_matrix_shaper` で代替）
...
| `formatter_for_colorspace()` | `cmsFormatterForColorspaceOfProfile` | `transform/samp.rs` (内部) |
