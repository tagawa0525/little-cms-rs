...
| C版                         | Rust版                                                         | 備考                         |
| --------------------------- | -------------------------------------------------------------- | ---------------------------- |
| `cmsS15Fixed16Number` (i32) | `S15Fixed16` newtype                                           | `From<f64>`/`Into<f64>` 実装 |
...
impl PixelFormat {
    // ビルダー: colorspace/channels/bytes から生成
    pub const fn build(colorspace: u32, channels: u32, bytes: u32) -> Self { ... }
    // チェーン可能なwithメソッド
    pub const fn with_extra(self, e: u32) -> Self { ... }
    pub const fn with_doswap(self, v: bool) -> Self { ... }
    // ... with_swapfirst, with_flavor, with_planar, with_endian16, with_float, with_premul
    // フィールドgetter（&self）
    pub const fn colorspace(&self) -> u32 { ... }
    pub const fn channels(&self) -> u32 { ... }
    pub const fn bytes(&self) -> u32 { ... }
    // ... extra, doswap, swapfirst, flavor, planar, endian16, is_float, optimized, premul
}

// 定義済みフォーマット定数（~170個）
pub const TYPE_RGB_8: PixelFormat = PixelFormat::build(...);
pub const TYPE_CMYK_16: PixelFormat = PixelFormat::build(...);
```
...
- ルックアップテーブルではなく直接ビット演算で実装（C版 `cmshalf.c` と同等の変換結果）
- `f32::to_bits()` / `f32::from_bits()` でビットパターン操作（C版の `union` 相当）
- ゼロ・正規化数・非正規化数・Inf・NaN の各ケースを分岐処理

#### テスト（GREEN後の状態）

- 特殊値: `+0.0`, `-0.0`, `1.0`, `-1.0`, `+Inf`, `-Inf`, NaN
- 正規数round-trip: `[0.5, 0.25, 2.0, 100.0, 0.001, 65504.0]`
- 非正規数: 最小正の非正規half（`0x0001`）が正の値かつ `<1e-6` であること
- 最大有限値: `65504.0 ↔ 0x7BFF`
...
| `cmsMD5finish`    | `Md5::finish(self) -> [u8; 16]`       | `self`を消費。`Md5::digest(data) -> [u8; 16]` も追加 |
...
| C版関数      | Rust版                                            | 備考                                |
| ------------ | ------------------------------------------------- | ----------------------------------- |
| `cmsXYZ2xyY` | `xyz_to_xyy(xyz: &CieXyz) -> CieXyY`              |                                     |
| `cmsxyY2XYZ` | `xyy_to_xyz(xyy: &CieXyY) -> CieXyz`              |                                     |
| `cmsXYZ2Lab` | `xyz_to_lab(wp: &CieXyz, xyz: &CieXyz) -> CieLab` | C版のNULL→D50デフォルトは引数で明示 |
| `cmsLab2XYZ` | `lab_to_xyz(wp: &CieXyz, lab: &CieLab) -> CieXyz` |                                     |
| `cmsLab2LCh` | `lab_to_lch(lab: &CieLab) -> CieLCh`              |                                     |
| `cmsLCh2Lab` | `lch_to_lab(lch: &CieLCh) -> CieLab`              |                                     |

**Lab/XYZエンコード・デコード**（自由関数として実装）

| C版関数               | Rust版                                               | 備考                |
| --------------------- | ---------------------------------------------------- | ------------------- |
| `cmsFloat2LabEncoded` | `float_to_pcs_encoded_lab(lab: &CieLab) -> [u16; 3]` | V4エンコーディング  |
| `cmsLabEncoded2Float` | `pcs_encoded_lab_to_float(enc: &[u16; 3]) -> CieLab` |                     |
| `cmsFloat2XYZEncoded` | `float_to_pcs_encoded_xyz(xyz: &CieXyz) -> [u16; 3]` | u1Fixed15Number形式 |
| `cmsXYZEncoded2Float` | `pcs_encoded_xyz_to_float(enc: &[u16; 3]) -> CieXyz` |                     |

V2エンコーディング・複数DeltaEバリアントは上位フェーズで必要になった時点で追加。

**DeltaE**（Phase 1はCIE76のみ）

| C版関数            | Rust版                                         | 備考              |
| ------------------ | ---------------------------------------------- | ----------------- |
| `cmsDeltaE`        | `delta_e(lab1: &CieLab, lab2: &CieLab) -> f64` | CIE76のみ実装     |
| `cmsCIE94DeltaE`   | Phase 2以降                                    | gamma/intrpに依存 |
| `cmsBFDdeltaE`     | Phase 2以降                                    |                   |
| `cmsCMCdeltaE`     | Phase 2以降                                    |                   |
| `cmsCIE2000DeltaE` | Phase 2以降                                    |                   |

**ユーティリティ**（Phase 1の範囲外）

`cmsD50_XYZ`, `cmsChannelsOfColorSpace`, `_cmsICCcolorSpace` 等のユーティリティ関数は
D50定数（`D50_X/Y/Z`）とColorSpaceSignatureが実装されているため、
上位フェーズで必要になった時点で追加する。
